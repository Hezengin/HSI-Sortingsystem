...
def init_cam_parameters(ui_context, frame_rate=15.0, exposure_time=30000.0, r=100, g=120, b=140, binning_type="BinningHorizontal", binning=2):
    ...
    ui_context["log_func"](f"[INFO] Camera initialized with parameters: FPS: {frame_rate} , EXP time: {exposure_time}, RGB: {r}, {g}, {b}")

def start_datacube(ui_context, preview=True):
    init_cam_parameters(ui_context, 15.0, 30000.0, 100, 120, 140, "BinningHorizontal", 2)
    cam = ui_context["camera_data"].get("cam")
    if not cam:
...
def stop_datacube(ui_context):
    ...
    save_path = os.path.join(folder, f"data_cube_{timestamp}.npy")
...
def create_datacube_with_duration(ui_context, preview=True, duration=2.0):
    ...
    def worker():
        ...
        if not cam._check_connection():
            ui_context["log_func"](LogLevel.ERROR,"Camera connection not active. Open Stream failed")
            return
        
        start_datacube(ui_context, preview)
        ui_context["log_func"](LogLevel.INFO,"Starting datacube acquisition for {duration:.1f} seconds...")
        time.sleep(duration)
...
def close(ui_context):
    cam = ui_context["camera_data"].get("cam")
    camera_data = ui_context["camera_data"]
